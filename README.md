# OS Lab 3 - Межпроцессное взаимодействие

## Описание проекта

Программа демонстрирует межпроцессное взаимодействие (IPC) через:
- **Memory-mapped files** (отображаемые файлы) для передачи данных
- **POSIX Named Semaphores** (именованные семафоры) для синхронизации

Родительский процесс читает числа от пользователя, передаёт их дочернему процессу через отображённый в память файл. Дочерний процесс суммирует числа и записывает результат в выходной файл.
...
## 1. MEMORY-MAPPED FILES (Отображаемые файлы)

### Что это?

**Memory-mapped file** — это механизм, при котором файл на диске отображается напрямую в адресное пространство процесса. Работа с памятью = работа с файлом.

---

### Создание файла (`parent.c`)
...
- **Заголовок:** `<unistd.h>`
- **Примечание:** Если файл не существует, вернёт `-1` (не критично)
...
```
- **Назначение:** Открывает или создаёт файл
- **Параметры:**
  - `path` — путь к файлу (`"shared_data.bin"`)
  - `flags` — флаги открытия (комбинируются через `|`):
    - **`O_RDWR`** — открыть для чтения и записи
    - **`O_CREAT`** — создать файл, если не существует
    - **`O_TRUNC`** — обрезать файл до 0 байт (очистить содержимое)
    - **`O_RDONLY`** — только чтение (не используется здесь)
    - **`O_WRONLY`** — только запись (используется для выходного файла)
...
```
- **Назначение:** Устанавливает размер файла
- **Параметры:**
  - `fd` — файловый дескриптор из `open()`
  - `length` — новый размер файла в байтах (`4096` в нашем случае)
- **Возврат:** `0` при успехе, `-1` при ошибке
- **Заголовок:** `<unistd.h>`
- **Важно:** Без `ftruncate` файл имеет размер 0, и `mmap` не сможет работать!
...
shared_data_t* shared_data = mmap(NULL, SHM_SIZE, 
                                 ...
                                 MAP_SHARED, fd, 0);
```
- **Назначение:** Отображает файл или объект в память процесса
- **Параметры:**
  ...
  - **`fd`** — файловый дескриптор (из `open`)
  - **`offset`** — смещение от начала файла (обычно `0`)
...
### Открытие существующего файла (`child.c`)

```c
int fd = open(MMAP_FILE, O_RDWR, 0644);
```
- **Отличие:** Без флага `O_CREAT` — только открывает существующий файл
- Если файл не существует → ошибка (`fd == -1`)
- Родитель создал, ребёнок подключается
...
```
- **Назначение:** Отключает отображение памяти
- **Параметры:**
  - `addr` — адрес начала (вернулся из `mmap`)
  - `length` — размер области
- **Возврат:** `0` при успехе, `-1` при ошибке
- **Заголовок:** `<sys/mman.h>`
- **Важно:** Освобождает виртуальную память процесса, но **не удаляет файл**!
...
3. Создаёт файл: `open()` → `ftruncate()` → `mmap()`
...
2. Открывает memory-mapped file: `open()` → `mmap()`
...
- Очищает ресурсы: `munmap()`, `unlink()`, `sem_close()`, `sem_unlink()`
...
### **Memory-mapped file vs POSIX Shared Memory:**

| Характеристика | Memory-mapped file | Shared Memory |
|----------------|-------------------|---------------|
| **Создание** | `open()` | `shm_open()` |
| **Где хранится** | Файл на диске | Объект в RAM |
| **Персистентность** | Да | Нет |
| **Путь** | `/path/to/file.bin` | `/shm_name` |
| **Удаление** | `unlink(path)` | `shm_unlink(name)` |
...
- **`-1`** — большинство функций (`open`, `read`, `write`, и т.д.)
- **`NULL`** — указатели (`mmap` возвращает `MAP_FAILED`)
- **`SEM_FAILED`** — `sem_open()`
...
POSIX семафоры (`sem_*`) входят в библиотеку `libpthread`, поэтому компилятор должен знать, где искать реализацию этих функций.
...
│                    PARENT PROCESS                        │
│                                                          │
│  1. Создаёт memory-mapped file (shared_data.bin)        │
...
│                    CHILD PROCESS                         │
│                                                          │
│  1. Открывает memory-mapped file                        │
│  2. Открывает семафоры                                  │
│  3. sem_wait(sem_parent) → ждёт данных                  │
│  4. Читает данные из файла                              │
...
- **Memory-mapped file** — файл, отображённый в память
- **Semaphore** — примитив синхронизации со счётчиком
- **Named semaphore** — семафор с именем для несвязанных процессов
- **File descriptor** — целое число, идентифицирующее открытый файл
